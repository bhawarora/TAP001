...
def test_closepromptvideo_flow(page):
    ...
    # Close How to Write Effective Prompts window by clicking the cross X icon at the top right
    # Try to locate the X icon using its SVG or button role, since it is visually present in the screenshot
    close_icon = None
    try:
        # Try SVG with stroke or path, or button with role and text
        close_icon = frame.locator("svg[aria-label*='Close'], svg[title*='Close'], svg, button, [role='button']").filter(lambda el: el.is_visible() and (el.inner_text().strip() in ['×', 'x', 'X'] or 'close' in (el.get_attribute('aria-label') or '').lower()))
        if close_icon.count() > 0:
            close_icon.nth(0).click()
        else:
            # Try clicking at coordinates (top right corner)
            bounding_box = frame.locator("body").bounding_box()
            if bounding_box:
                page.mouse.click(bounding_box['x'] + bounding_box['width'] - 10, bounding_box['y'] + 10)
            else:
                raise AssertionError("Could not find or click the close icon for the prompt dialog.")
    except Exception:
        # Fallback: click at top right corner
        bounding_box = frame.locator("body").bounding_box()
        if bounding_box:
            page.mouse.click(bounding_box['x'] + bounding_box['width'] - 10, bounding_box['y'] + 10)
        else:
