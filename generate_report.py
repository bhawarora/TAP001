...
def generate_html_report(results_file=None, output_file=None):
    ...
    skipped_steps = sum(1 for r in results if r["status"] == "SKIP")
    ...
    for result in results:
        ...
        # Determine test status: FAIL if any FAIL, else SKIP if any SKIP, else PASS
        if result["status"] == "FAIL":
            tests_by_id[test_id]["status"] = "FAIL"
        elif result["status"] == "SKIP" and tests_by_id[test_id]["status"] != "FAIL":
            tests_by_id[test_id]["status"] = "SKIP"
    
    # Calculate test stats
    total_tests = len(tests_by_id)
    passed_tests = sum(1 for t in tests_by_id.values() if t["status"] == "PASS")
    failed_tests = sum(1 for t in tests_by_id.values() if t["status"] == "FAIL")
    skipped_tests = sum(1 for t in tests_by_id.values() if t["status"] == "SKIP")
    # For display consistency, total_tests = passed + failed + skipped
    # pass_rate based on executed tests (excluding skipped) if desired; keeping original behavior using total_tests
